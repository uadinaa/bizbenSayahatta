...
export default function PublicTripsSection({loadingTrips, publicTrips }) {
  ...
  return (
    <>
      ...
      {loadingTrips && <div className={styles.sectionNote}>{t("inspiration.card.loadingTrips")}</div>}
      {!loadingTrips && publicTrips.length === 0 && (
        <div className={styles.sectionNote}>{t("inspiration.card.noApprovedTripsYet")}</div>
      )}
          ...
          {publicTrips.map((trip) => (
            <div key={trip.id} className={styles.tripCard}>
              ...
              <div className={styles.tripFooter}>
                <span>{trip.duration_days} {t("inspiration.card.days")}</span>
                <span>{trip.price ? `$${trip.price}` : t("inspiration.card.contact")}</span>
              </div>
            </div>
          ))}
        </div>
